...
export const allEntries = async (userId: number): Promise<oldEntryT[] | null> => {
...
export const getEntryById=async(entry_id:number):Promise<oldEntryT|null>=>{
  const api:string=`${process.env.EXPO_PUBLIC_API_BASE_URL}/entries/entry/${entry_id}`
  try{
    const response=await axios.get(api);
    // console.log("api: ",api)
    // console.log("response: ",response.data.entry);
...
const convertAPIResponseTOldEntryT=(apiResEntry:any):oldEntryT=>{
  return {
    ...
    datetime: apiResEntry.created_at,
